...
EM_OPT            := -s STANDALONE_WASM -s NO_DYNAMIC_EXECUTION=1 -s MODULARIZE=1 -s EXPORT_ES6=1 --minify 0 -s INVOKE_RUN=0 -s FILESYSTEM=0 -s EXPORT_NAME=exorbitant
EM_EXPORTS        := -s EXTRA_EXPORTED_RUNTIME_METHODS="['stackAlloc', 'stackSave', 'stackRestore', 'stringToUTF8']"
OPTIMIZATION_OPT  := -O3
